const ms = require('ms')
const { warn_2 } = require('../../lib')
module.exports = {
	args: 1,
	permissions: ['MUTE_MEMBERS'],
	description: 'Keqing will warn member has mentioned (second warn)',
	run (bot, message, args) {
		...
		try {
			member.roles.add(warn_2)
			message.channel.send(`Keqing has warned <@${member.user.id}> ${args[1] ? `for ${ms(ms(args[1]), { long: true })} ` : ''}:( Sorry ;-; (second warn)`)
			if (args[1]) {
				setTimeout(() => {
					member.roles.remove(warn_2)
